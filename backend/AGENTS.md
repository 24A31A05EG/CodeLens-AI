...
Do not describe deterministic fallback output as production LLM output. The current application is useful without a model provider, but the the optional Gemini call remains disabled until configured.
...
3. A FastAPI background task runs `parse_project`, then the static-analysis pipeline (`services/analysis/pipeline.py`): per-file facts -> relationship graph -> project context.
4. Supported files become `ProjectFile` rows with language, symbols, and imports (derived from the analysis when it succeeds). Secret files (`.env`, keys), lockfiles and minified/oversized files are skipped.
5. Each file is chunked (secrets redacted) and each chunk receives a deterministic local embedding; chunks are stored in `Chunk` rows. Per-file facts and the project graph/context are stored in `FileAnalysis` / `ProjectAnalysis`.
6. The project becomes `ready` or `failed`. An analysis failure does not fail the project; legacy behaviour is used as fallback.
7. The frontend polls `/upload/{project_id}/status` and then loads `/projects/{project_id}/structure`.
8. Explanation requests use the stored analysis (file facts + graph neighbours + project context) and cache results in `Explanation`. Only ingested files can be explained.
9. Documentation and diagram requests are generated from the stored analysis and cached in `GeneratedArtifact`. A cached row older than the project's analysis is stale and is regenerated.
10. Q&A detects the question intent, ranks files (path, symbols, tags, chunk text, graph neighbourhood) and composes an answer from the graph and facts; sources include the reason for each hit. The old embedding/cosine path only runs if no analysis can be built.
...
The runtime can optionally call Google Gemini when enabled. In `backend/services/ai_client.py`:

- `_call_model()` calls Google Gemini when enabled and configured, otherwise returns `None`.
- `GEMINI_API_KEY` and `CODELENS_ENABLE_GEMINI` control the optional provider integration.
...
- Detects files by extension using `EXT_TO_LANG`, plus a few well-known names (`Dockerfile`, `requirements*.txt`) via `language_for()`. Arbitrary `.txt` remains unsupported.
- Skips secret files, lockfiles, minified and oversized files.
- Chunks at function/class boundaries reported by the analyzer (keeping an imports preamble chunk); otherwise uses 60-line windows with 10-line overlap. All chunks pass through `redact_secrets`.

Static analysis (`backend/services/analysis/`):

- `extract_py.py` uses the standard `ast` module (high confidence). `extract_js.py` is regex-based with a comment/string-aware scanner (medium confidence, not a full parser). `extract_misc.py` covers JSON/package.json/tsconfig, YAML/compose, TOML, Dockerfile, requirements, Markdown, HTML, CSS/SCSS, SQL; other languages are low-confidence pattern matching.
- `graph.py` resolves imports inside the project and builds typed edges with evidence. Frontend-to-backend API links are matched by URL and always marked `inferred`.
- `context.py` builds the project overview; `explainer.py`, `retrieval.py`, `docgen.py` produce explanations, Ask answers, docs and diagrams.
- `store.py` persists and lazily rebuilds the analysis (`ensure_analysis`). Bump `ANALYSIS_VERSION` in `pipeline.py` when analyzer output changes.
- No language model is involved: `_call_model()` is still a stub. `build_file_prompt()` prepares a bounded, redacted single-project prompt for when it is wired.

The legacy Q&A path in `backend/routers/ask.py` (fallback only, when no analysis exists):

- Embeds the question with `embed_text()` (a mock embedding, not semantic).
...
Do not claim that JS/TS analysis is a full parser, that API links are certain, or that any semantic model or vector search is used. Those are potential future improvements, not current functionality.
...
- `ProjectAnalysis`: per-project graph and context JSON plus analyzer version (one row per project).
- `FileAnalysis`: per-file extracted facts JSON.
...
- `GEMINI_API_KEY`: Gemini API credential; keep it out of source control.
- `GEMINI_MODEL`: model identifier; defaults to `gemini-3.8-flash`.
- `CODELENS_ENABLE_GEMINI`: defaults to `0`; set to `1` to enable Gemini.
...
- Treat `GEMINI_API_KEY` and `GEMINI_API_KEY` as sensitive even though the current runtime does not use them.
